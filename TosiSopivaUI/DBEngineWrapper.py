...
class DBEngineWrapper():
    ...
    def getCustomer(self, customer_id):
        ...
        if json_data_ptr.value is not None:
            raw_data = json_data_ptr.value
            detected_encoding = chardet.detect(raw_data)['encoding']
            decoded_data = raw_data.decode(detected_encoding) if detected_encoding else raw_data.decode('utf-8', errors='ignore')
            customer_data = json.loads(decoded_data)
        else:
            customer_data = {}
